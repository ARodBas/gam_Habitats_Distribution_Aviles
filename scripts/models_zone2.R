...
## Load pres-abs data
sps_data <- readRDS("data/pres_abs/presabs_Zone2.rds")
...
Model_Full <- mgcv::gam(PresAus ~ s(BPI_f,k=4) + s(BPI_b,k=4) + s(Depth,k=4) + s(Backscatter,k=4) + s(Slope,k=4) + s(East,k=4) + s(North,k=4) + as.factor(Geomorf), family= binomial, data=sps_data)
...
## Remove Geomorf For model 3. Better predictions
if (j == 3) {
  ModelSelect_Final <- mgcv::gam(PresAus ~ s(BPI_f,k=4) + s(BPI_b,k=4) + s(Depth,k=4) + s(Backscatter,k=4) + s(East,k=4) + s(North,k=4) + s(Curv,k=4) + s(Slope,k=4), family= binomial, data=sps_data)
}
...
All_DF <- readRDS("data/predictors/predictors_df_zone2.rds")
...
if (is.null(FinalGAM$xlevels$`as.factor(Geomorf)`)) {All_SPDF_b <- p_sdf; All_DF <- as.data.frame(All_SPDF_b)} else {
  All_SPDF_b <- All_SPDF[which(p_sdf@data$Geomorf %in% FinalGAM$xlevels$`as.factor(Geomorf)`),]
...
## Remove shallower data for j==1
if(j==1){All_SPDF_new$GAM_bin[which(All_SPDF_new$Depth<300)] <- 0}
...
## Save raster
out_file <- paste0(
  "outputs/pres_abs/GAM_z2_presaus_th_",j,".tif"
)
...
dens_data <- readRDS("data/densities/sps_dens_mat_z2.rds")
...
#############################################################################################
## Group 3 does not fit with geomorphology,
## we remove it from the final model
#############################################################################################


if (j==3) {
  Model_Full <- mgcv::gam(dens ~ s(BPI_f,k=4) + s(BPI_b,k=4) + s(Backscatter,k=4) + s(Slope,k=4) + s(East,k=4) + s(North,k=4), family= "nb", data=dens_data)
...
## get delta data (pres-abs * densities)
...
## Save delta map
out_file <- paste0(
  "outputs/delta/gam_z2_delta_",j,".tif"
)
